...
export const useLeaderboard = (personalBest: number) => {
  ...
  const fetchLeaderboard = useCallback(async () => {
    ...
    try {
      ...
      const playerInTopEntries =
        userId && topScores?.some((entry) => entry.user_id === userId);

      if (!playerInTopEntries && userId) {
        // Get player rank if not in top 100 and userId is present
        ...
        const playerRank =
          topScores.findIndex((entry) => entry.user_id === userId) + 1;
  ...
  const updateLeaderboardEntry = async (nickname: string, score: number) => {
    ...
    try {
      logger.info(
        `Attempting to update leaderboard for ${nickname} with score ${score}`
      );
      ...
      if (!userId) {
        ...
        if (error) {
          ...
          setUpdating(false);
          setError(
            "Failed to create leaderboard entry. Please try again later."
          );
          return;
        }
        ...
        const { data, error } = await supabase.rpc("update_leaderboard_score", {
          uid: userId,
          score: score,
        });
        if (error) {
          ...
          setUpdating(false);
          setError(
            "Failed to update leaderboard entry. Please try again later."
          );
          return;
        }
