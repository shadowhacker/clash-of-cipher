...
const GameStatus: React.FC<GameStatusProps> = ({ 
  ...
  return (
    <div className="flex flex-col space-y-1 max-w-[320px] mx-auto">
          ...
          <div className="text-sm text-amber-400 font-medium text-center w-full">
            {gameState === 'idle' && playerName ? (
              <div className="space-y-1">
                ...
                <div>🏆 Best Meditation: {personalBest}</div>
              </div>
            ) : (
              <div>🏆 Best Meditation: {personalBest}</div>
            )}
      ...
      <div className="h-[48px] flex items-center justify-between mb-2 bg-amber-950/50 rounded-md p-2 transition-colors border border-amber-800/30">
        {gameState === 'idle' ? (
          ...
          <>
            ...
            <div className="flex items-center space-x-4">
              ...
              <span className={`text-amber-400 font-medium ${isTimeCritical ? 'animate-bounce' : ''}`}>
                ⏱ {timeLeft}s
              </span>
              <Button 
                variant="ghost" 
                size="sm" 
                onClick={resetGame} 
                className="p-1 text-amber-400 hover:text-amber-300 hover:bg-amber-800/30"
              >
